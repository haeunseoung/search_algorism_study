...
int main()
{
    ...
    auto ais = std::array<StringAIPair, 2>{
        StringAIPair("mctsAction 3000", [](const State &state)
                     { return montecarlo::mctsAction(state, 3000); }),
        StringAIPair("mctsAction 30", [](const State &state)
                     { return montecarlo::mctsAction(state, 30); }),
    };
